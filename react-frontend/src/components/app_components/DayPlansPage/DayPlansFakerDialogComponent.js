...
import dayPlansFakerFactory from "./dayPlansFakerFactory";
...
const DayPlansFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dayPlansFakerFactory(props.user,fakerCount,ItineraryIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dayPlans").create(elem));
...
export default connect(mapState, mapDispatch)(DayPlansFakerDialogComponent);
