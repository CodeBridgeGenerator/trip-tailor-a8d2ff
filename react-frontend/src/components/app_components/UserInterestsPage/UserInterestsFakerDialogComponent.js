...
import userInterestsFakerFactory from "./userInterestsFakerFactory";
...
const UserInterestsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userInterestsFakerFactory(props.user,fakerCount,userIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userInterests").create(elem));
...
export default connect(mapState, mapDispatch)(UserInterestsFakerDialogComponent);
