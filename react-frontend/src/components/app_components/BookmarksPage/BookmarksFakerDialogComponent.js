...
import bookmarksFakerFactory from "./bookmarksFakerFactory";
...
const BookmarksFakerDialogComponent = (props) => {
...
client.service("itineraries")
    ...
    const onRun = async () => {
        let fakeData = bookmarksFakerFactory(props.user,fakerCount,userIDIds,itineraryIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("bookmarks").create(elem));
...
export default connect(mapState, mapDispatch)(BookmarksFakerDialogComponent);
