...
import activitiesFakerFactory from "./activitiesFakerFactory";
...
const ActivitiesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [dayPlanIDIds, setdayPlanIDIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("dayPlans")
                        .find({ query: { $select: ["_id","date"] } })
                        .then((data) => {
                        setdayPlanIDIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = activitiesFakerFactory(props.user,fakerCount,dayPlanIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("activities").create(elem));
...
export default connect(mapState, mapDispatch)(ActivitiesFakerDialogComponent);
