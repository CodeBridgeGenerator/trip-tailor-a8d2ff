...
import itinerariesFakerFactory from "./itinerariesFakerFactory";
...
const ItinerariesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = itinerariesFakerFactory(props.user,fakerCount,userIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("itineraries").create(elem));
...
export default connect(mapState, mapDispatch)(ItinerariesFakerDialogComponent);
