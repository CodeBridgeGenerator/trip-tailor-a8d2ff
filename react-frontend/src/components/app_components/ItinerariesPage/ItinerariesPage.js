...
import ItinerariesDatatable from "./ItinerariesDataTable";
import ItinerariesEditDialogComponent from "./ItinerariesEditDialogComponent";
import ItinerariesCreateDialogComponent from "./ItinerariesCreateDialogComponent";
import ItinerariesFakerDialogComponent from "./ItinerariesFakerDialogComponent";
import ItinerariesSeederDialogComponent from "./ItinerariesSeederDialogComponent";
...
const ItinerariesPage = (props) => {
    ...
    const filename = "itineraries.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("itineraries");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("itineraries").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Itineraries", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("itineraries").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="itineraries-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="itineraries-datatable">
                    <ItinerariesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ItinerariesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ItinerariesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ItinerariesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ItinerariesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ItinerariesPage);
