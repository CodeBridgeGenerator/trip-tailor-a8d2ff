...
import interestsFakerFactory from "./interestsFakerFactory";
...
const InterestsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = interestsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("interests").create(elem));
...
export default connect(mapState, mapDispatch)(InterestsFakerDialogComponent);
