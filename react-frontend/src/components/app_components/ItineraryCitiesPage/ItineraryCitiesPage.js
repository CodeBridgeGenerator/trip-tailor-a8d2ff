...
import ItineraryCitiesDatatable from "./ItineraryCitiesDataTable";
import ItineraryCitiesEditDialogComponent from "./ItineraryCitiesEditDialogComponent";
import ItineraryCitiesCreateDialogComponent from "./ItineraryCitiesCreateDialogComponent";
import ItineraryCitiesFakerDialogComponent from "./ItineraryCitiesFakerDialogComponent";
import ItineraryCitiesSeederDialogComponent from "./ItineraryCitiesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const ItineraryCitiesPage = (props) => {
    ...
    const filename = "itineraryCities.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("itineraryCities");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , itineraryID : urlParams.singleItinerariesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "ItineraryCities", type: "error", message: error.message || "Failed get ItineraryCities" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("itineraryCities").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "ItineraryCities", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("itineraryCities").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "ItineraryCities",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/itineraryCities/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="itineraryCities-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="itineraryCities-datatable">
                    <ItineraryCitiesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <ItineraryCitiesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ItineraryCitiesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <ItineraryCitiesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ItineraryCitiesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ItineraryCitiesPage);
