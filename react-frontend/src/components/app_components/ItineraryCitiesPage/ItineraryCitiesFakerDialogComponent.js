...
import itineraryCitiesFakerFactory from "./itineraryCitiesFakerFactory";
...
const ItineraryCitiesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [itineraryIDIds, setitineraryIDIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("itineraries")
                        .find({ query: { $select: ["_id","title"] } })
                        .then((data) => {
                        setitineraryIDIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = itineraryCitiesFakerFactory(props.user,fakerCount,itineraryIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("itineraryCities").create(elem));
...
export default connect(mapState, mapDispatch)(ItineraryCitiesFakerDialogComponent);
